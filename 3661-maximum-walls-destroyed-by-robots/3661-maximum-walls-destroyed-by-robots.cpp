...
public:
    ...
    int solve(vector<int>& walls, vector<P>& roboDist, vector<P>& range, int i, int prevDir) {
        ...
        int leftTake = count(walls, leftStart, roboDist[i].first) + solve(walls, roboDist, range, i+1, 0);      
        int rightTake = count(walls, roboDist[i].first, range[i].second) + solve(walls, roboDist, range, i+1, 1);
